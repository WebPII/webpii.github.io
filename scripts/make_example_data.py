...
Usage:
    python scripts/make_example_data.py                    # Use defaults
    python scripts/make_example_data.py --num-products 50  # Limit product count
    python scripts/make_example_data.py --num-images 5     # Limit UI screenshot count
    python scripts/make_example_data.py --clean            # Remove existing example_data first
"""
...
def copy_product_data(num_products: int = DEFAULT_NUM_PRODUCTS, num_images: int = DEFAULT_NUM_PRODUCT_IMAGES):
    ...
    if not products_merged.exists():
        ...
        print("  Run 'python scripts/merge_product_data.py' first to create it")
        return 0, 0
